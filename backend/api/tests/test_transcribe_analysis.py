"""
Test script for TranscribeAnalysisImageView

This script can be run standalone to test the transcription functionality
without starting the full Django server.

Usage:
    python -m api.test.test_transcribe_analysis
"""
import os
import sys
import django
...
from api.views.transcribe_analysis import TranscribeAnalysisImageView
from api.models import AnalysisTranscript
...
async def test_transcribe_with_image():
    ...
    """
    print("=" * 60)
    print("Testing TranscribeAnalysisImageView with image...")
    print("=" * 60)
    ...
    view = TranscribeAnalysisImageView()
...
async def test_transcribe_with_text_fallback():
    ...
    """
    print("\n" + "=" * 60)
    print("Testing TranscribeAnalysisImageView with text fallback...")
    print("=" * 60)
...
async def test_transcribe_no_data():
    ...
    """
    print("\n" + "=" * 60)
    print("Testing TranscribeAnalysisImageView with no data (should fail)...")
    print("=" * 60)
    
    # Create a mock request without image or text
    factory = RequestFactory()
    request = factory.post('/api/transcribe-analysis/', {})
    request.session = {}
...
async def cleanup_test_data():
    ...
    try:
        ...
        count = await AnalysisTranscript.objects.all().adelete()
